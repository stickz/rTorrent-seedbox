...
utils::Directory
path_expand_transform(std::string path, const dir_utils::directory_entry& entry) {
...
path_expand(std::vector<std::string>* paths, const std::string& pattern) {
  ...
  for (; first != last; ++first) {
    ...
    for (std::vector<utils::Directory>::iterator itr = currentCache.begin(); itr != currentCache.end(); ++itr) {
      ...
      itr->erase(std::remove_if(itr->begin(), itr->end(), rak::on(rak::mem_ref(&dir_utils::directory_entry::d_name), std::not1(r))), itr->end());

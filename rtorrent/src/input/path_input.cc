...
struct _transform_filename {
  void operator () (directory_entry& entry) {
#ifdef __sun__
...
PathInput::receive_do_complete() {
  ...
  Directory dir(dirEnd != 0 ? str().substr(0, dirEnd) : "./");
  
  if (!dir.update(Directory::update_sort | Directory::update_hide_dot) || dir.empty()) {
    mark_dirty();
  ...
  std::string base = rak::make_base<std::string>(r.first, r.second, rak::const_mem_ref(&directory_entry::d_name));
  ...
  m_showNext = ++Directory::iterator(r.first) != r.second;
...
inline bool
find_complete_compare(const directory_entry& complete, const std::string& base) {
  return complete.d_name.compare(0, base.size(), base);
}

inline bool
find_complete_not_compare(const directory_entry& complete, const std::string& base) {
  return !complete.d_name.compare(0, base.size(), base);
}

PathInput::range_type
PathInput::find_incomplete(Directory& d, const std::string& f) {
  range_type r;

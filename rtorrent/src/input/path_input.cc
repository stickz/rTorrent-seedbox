...
struct _transform_filename {
  void operator () (dir_utils::directory_entry& entry) {
#ifdef __sun__
...
PathInput::receive_do_complete() {
  ...
  std::string base = rak::make_base<std::string>(r.first, r.second, rak::const_mem_ref(&dir_utils::directory_entry::d_name));
...
inline bool
find_complete_compare(const dir_utils::directory_entry& complete, const std::string& base) {
  return complete.d_name.compare(0, base.size(), base);
}

inline bool
find_complete_not_compare(const dir_utils::directory_entry& complete, const std::string& base) {
  return !complete.d_name.compare(0, base.size(), base);
}
